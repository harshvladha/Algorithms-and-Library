...
void primsMST(Graph g, int src){
	...
	for(vx=0;vx<v;vx++){
		distance[vx] = INT_MAX;
		heap->array[vx] = newHNode(vx, distance[vx]);
		heap->pos[vx] = vx;
	}

	//make distance value of src vertex as 0 so that it is extracted first
	heap->array[src] = newHNode(src, distance[src]);
	heap->pos[src] = src;
	distance[src] = 0;
	...
	while(!isEmpty(heap)){
		...
		while(pCrawl != NULL){
			...
			if(isInMinHeap(heap, vx) && distance[u]!=INT_MAX && pCrawl->weight < distance[vx]){
				distance[vx] = pCrawl->weight;
