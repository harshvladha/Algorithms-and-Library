...
void decreaseKey(MinHeap* heap, int v, int distance){
	...
	while(i && heap->array[i]->distance < heap->array[(i-1)/2]->distance){
		...
		heap->pos[haep->array[i]->v] = (i-1)/2;
		heap->pos[haep->array[(i-1)/2]->v] = i;
...
void dijsktra(Graph g, int src){
	int v = g.getV();
	int distance[v]; //distance values used to pick minimum weight edge in cut
	...
	for(int vx=0;vx<v;vx++){
		distance[vx] = INT_MAX;
		heap->array[vx] = newHNode(src, distance[vx]);
		heap->pos[vx] = vx;
	}

	//make distance value of src vertex as 0 so that it is extracted first
	heap->array[src] = newHNode(vx, distance[src]);
	heap->pos[src] = src;
	distance[src] = 0;
	decreaseKey(heap, src, distance[src]);
	...
	while(!isEmpty(heap)){
		...
		while(pCrawl != NULL){
			...
			if(isInMinHeap(heap, vx) && distance[u]!=INT_MAX && pCrawl->weight + distance[u] < distance[vx]){
				distance[vx] = distance[u] + pCrawl->weight;
				//update the distance value in min heap also
				decreaseKey(heap, vx, distance[vx]);
			}
			pCrawl = pCrawl->next;
		}
	}
